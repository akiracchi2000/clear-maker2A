...
読み取った解答と正誤:
（各問題の最終的な解答（数値や式）のみを箇条書きで示してください。**計算過程や途中式は絶対に含めないでください。** 形式は「(1) [答え] ○」または「(1) [答え] ×」のように、答えの右側に正誤記号を添えてください。数式には必ずKaTeX形式を用いてください）

フィードバック:
（不正解の問題がある場合のみ、その問題番号を明記した上で、生徒が間違えた原因を短潔に指摘し、途中式を含めた正しい解答例を出力してください。正答した問題の解説は一切不要です。全問正解の場合は簡潔なお祝いの言葉のみを記載してください。数式には必ずKaTeX形式を用いてください）
`;
