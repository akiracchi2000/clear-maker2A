...
読み取った解答と正誤:
（AIが各問題に対してどのように生徒の最終的な答えを読み取ったか、正誤（○または×）とともにテキストで示してください。途中式は含めず、最終的な答えのみを簡潔に示してください。形式は「(1) ○：[最終的な答え]」のようにしてください。数式には必ずKaTeX形式を用いてください）

フィードバック:
（不正解の問題がある場合のみ、その問題番号を明記した上で、生徒が間違えた原因を指摘し、途中式を含めた正しい解答例を出力してください。正解した問題については解説を省略してください。全問正解の場合は簡潔なお祝いの言葉を記載してください。数式には必ずKaTeX形式を用いてください）
`;
